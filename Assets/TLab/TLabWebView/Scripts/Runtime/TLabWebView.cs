...
namespace TLab.Android.WebView
{
	...
	public class TLabWebView : MonoBehaviour
	{
		...
		public void SetUrlAndLoad(string url)
		{
			m_url = url;  // Update the URL
			if (m_state == State.INITIALIZED)
			{
				LoadUrl(m_url);  // Load the URL if already initialized
			}
			else
			{
				Init();  // Otherwise, initialize which will load the URL
			}
		}
